...
SEARCH.get_shop_info = function(b, a) {
    ...
    $.ajax({url: "shop.php",data: {shop_id: b},dataType: "json",success: function(e) {
            ...
            if (!a) {
                ...
                if (e.logo) {
                    f.find(".shop-logo").html('<a href="../../js/' + e.url + '" title="' + e.title + '" target="_blank"><img data-img="2" width="135" height="45" data-lazyload="' + e.logo + '"></a>').show()
                } else {
                    f.find(".shop-logo").html('<a href="../../js/' + e.url + '" title="' + e.title + '" target="_blank"></a>').show()
                }
...
SEARCH.get_shop_name = function() {
    ...
    $.getJSON("ShopName.php", {ids: a.unique().join(",")}, function(n) {
        ...
        for (var k = 0; k < d; k++) {
            ...
            if (m != "1") {
                var l = '<a target="_blank" onclick="searchlog(1,' + p.id + ',0,58)" href="../../js/' + p.url + '" title="' + p.title + '">' + p.title + "</a>";
                m == "2" && (l = "由 " + l + " 发货");
                o.html(l);
...
SEARCH.page = function(b, a) {
    ...
    window.location = "http://www.atguigu.com/search.html?q="+SEARCH.query+"&page="+b;
//    if (a) {
...
SEARCH.load = function(b, a) {
    ...
    $.ajax({url: b.replace(/[\s&]*$/g, ""),timeout: 10000,error: function() {
            ...
            if (c == 1) {
                ...
                if (c == 3) {
                    $("#refilter").find("h2").html('<a href="../../js/' + f.top_url + '">查看所有类目</a>');
...
SEARCH.get_paipai_ware = function(c) {
    ...
    $.ajax({url: "http://sse1.paipai.com/comm_json?charset=utf-8&dtype=jsonp&dtag=search&KeyWord=" + b + "&PageSize=5&ShowClass=1",dataType: "jsonp",scriptCharset: "utf-8",success: function(h) {
            ...
            g += "共" + h.data.totalNum + '个商品<a target="_blank" href="../../js/' + a + '" onclick="log(\'paipai\',\'normal-clk-all\',QUERY_KEYWORD)">查看全部</a></div><ul class="lh">';
            for (var f = 0, e = h.data.list.length; f < e; f++) {
                ...
                g += '<li><div class="p-img"><a target="_blank" title="' + n + '" href="' + l + '"' + j + '><img height="130" width="130" alt="" src="../../js/' + m.img160 + '"></a></div>';
