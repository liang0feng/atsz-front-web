...
function renderAccordion(t) {
    ...
    for (var a = '{for item in data}<a target="_blank" class="fore${arguments[2]}" href="../../js/${item.href}">    <strong>${item.title}</strong>    <em>${item.desc}</em>    <img data-img="2" data-lazyload="${item.srcB}" alt="${item.alt}"></a>{/for}', n = 0; t.length > n; n++)
        e(n, t[n].list);
...
//    var t = function(t) {
//        if (0 !== t.type) {
//            var e = $("#header-2013"), i = $('<div id="jd-ept-link" style="height: 60px;margin-bottom:10px;"></div>'), a = "http://en.jd.com/", n = '<div class="w" clstag="homepage|keycount|home2013|ept"><a target="_blank" href="../../js/' + a + '" style="display: block;text-align: center;border: 2px solid #F0AF00;background: #FFFBDD url(/mycart/images/blank.gif);">' + '<img style="vertical-align: top;" class="loading-style2" data-img="1" src="http://img11.360buyimg.com/da/g14/M03/1E/18/rBEhVlNUvcEIAAAAAAAgBol2wPcAAMVkwEJL5gAACAe087.png" width="874" height="56" />' + "</a></div>
";
//            i.html(n), e.after(i)
//        }
//    };
                ...
                if (e && e["269"] && e["269"].length == n) {
                    ...
                    i(l, function(e) {
                        for (var i in a) {
                            ...
                            o += '                            <li>                                <div class="p-img ld">                                    <a target="_blank" href="../../js/' + l + '" title="' + c + '">                                        <img data-lazyload="' + h + '" width="150" height="150" alt="' + c + '" data-img="1" data-exposal-url="' + d + '" />                                    </a>                                </div>                                <div class="p-name">                                    <a target="_blank" href="../../js/' + l + '" title="' + c + '" style="">' + c + '</a>                                </div>                                <div class="p-price">                                    ' + e[n] + "                                </div>                            </li>"
                        }
...
BackPanelNew.prototype = {init: function(t) {
    	...
    	// 去掉右侧工具条
    	return ;
        var e = this, i = '<div class="{if newTag}bp-item-new{/if} bp-item bp-item-${cName}" data-top="${scrollTo}">    <a href="../../js/${href}" class="${cName}" target="${target}">${text}</a><s></s></div>', a = $(i.process(t));
...
(function() {
    ...
    }), pageConfig.TPL_MSlide = {itemsWrap: '<ul class="slide-items">{innerHTML}</ul>',itemsContent: '{for item in json}            {var v=pageConfig.FN_GetCompatibleData(item)}            <li class="hide" clstag="homepage|keycount|home2013|09a${parseInt(item_index)+2}"><a href="../../js/${v.href}" target="_blank" title="${v.alt}">                <img height="${v.height}" width="${v.width}" src="../../js/${v.src}" data-img="2" />            </a></li>        {/for}',controlsWrap: '<div class="slide-controls">{innerHTML}</div>',controlsContent: '{for item in json}            <span class="{if parseInt(item_index)==0}curr{/if}">${parseInt(item_index)+1}</span>        {/for}'}, pageConfig.TPL_FSlide = {itemsWrap: '<div class="slide-itemswrap"><div class="slide-items">{innerHTML}</div></div>',itemsContent: '{for item in json}            <div><a href="../../js/${item.href}" target="_blank"><img src="../../js/${item.src}" width="${item.width}" height="${item.height}" alt="${item.alt}" data-img="2"></a></div>        {/for}',controlsWrap: '<div class="slide-controls">{innerHTML}</div>',controlsContent: '{for item in json}            <span class="{if parseInt(item_index)==0}curr{/if}"><b></b></span>        {/for}'}, pageConfig.FN_SetPrice = function(t) {
        var e = t.split(".");
    ...
    }, pageConfig.TPL_Tabs = '<ul class="lh style3">        {for item in data}        {var x=pageConfig.FN_SetPrice(item.c)}        {var y=pageConfig.FN_SetImg(item.a,item.d)}        <li class="fore${parseInt(item_index)}">            <div class="p-img ld"><a target="_blank" href="../../js/${pageConfig.FN_GetUrl(item.f,item.a)}" title="${item.b}">{if item.e}${pageConfig.FN_SetPromotion(item.e)}{/if}<img height="100" width="100" alt="${item.b}" data-img="1" src="../../js/${y}"></a></div>            <div class="p-name"><a target="_blank" href="../../js/${pageConfig.FN_GetUrl(item.f,item.a)}" title="${item.b}">${item.b}</a></div>            <div class="p-price"><span>\uffe5</span>${x}</div>        </li>        {/for}    </ul>', pageConfig.TPL_Tabs_jp = '<ul class="lh style3">        {for item in data}        <li class="fore${parseInt(item_index)}">            <div class="p-img ld"><a target="_blank" href="../../js/${pageConfig.FN_GetUrl(item.f,item.a)}" title="${item.b}">{if item.e}${pageConfig.FN_SetPromotion(item.e)}{/if}<img height="100" width="100" alt="${item.b}" data-img="1" src="../../js/${pageConfig.FN_GetImageDomain(item.a)}${item.d}"></a></div>            <div class="p-name" style="height:1.5em;"><a target="_blank" href="../../js/${pageConfig.FN_GetUrl(item.f,item.a)}" title="${item.b}">${item.b}</a></div>            <div class="p-price"><span>${item.c}</span></div>        </li>        {/for}    </ul>', pageConfig.FN_Tabs = function(t, e) {
        var i = t.split(",");
        obj = {}, html = "";
        var a;
...
}(jQuery), pageConfig.TPL_MScroll = '<ul class="lh">    {for slide in list}    <li class="item fore${parseInt(slide_index)+1}" clstag="homepage|keycount|home2013|09b${parseInt(slide_index)+1}">        <a href="../../js/${slide.href}" target="_blank">            <img width="202" height="159" src="/mycart/images/blank.gif" class="loading-style2" data-lazyload="background:url(${slide.src}) no-repeat #fff center 0;" alt="${slide.alt}" /></a>    </li>    {/for}</ul>', $("#mscroll-list").imgScroll({width: screen.width >= 1200 ? 203 : 163,height: 159,visible: 3,step: 3,loop: !0,next: "#mscroll-ctrl-next",prev: "#mscroll-ctrl-prev",data: pageConfig.DATA_MScroll,template: pageConfig.TPL_MScroll}, function(t, e, i, a) {
    a.find("img").each(function() {
        var t = $(this).attr("data-lazyload");
        $(this).attr("style") || $(this).attr("style", t).removeAttr("data-lazyload")
    })
}), pageConfig.TPL_GScroll = '<ul class="lh">    {for slide in list}    {var x=pageConfig.FN_GetCompatibleData(slide)}        <li class="item fore${parseInt(slide_index)+1}">            <a href="../../js/${x.href}" target="_blank">                <img width="${x.width}" height="${x.height}" src="/mycart/images/blank.gif" style="background:url(../../js/${x.src}) no-repeat 0 0;" class="loading-style2" alt="${x.alt}" /></a>            <div class="p-detail">                <a target="_blank" class="btn-tuan" href="../../js/${x.href}">\u53c2\u56e2</a>                <span>\u56e2\u8d2d\u4ef7\uff1a</span><strong>\uffe5${x.price}</strong>            </div>        </li>    {/for}</ul>', $("#gscroll-list").imgScroll({width: screen.width >= 1200 ? 292 : 192,height: 155,visible: 1,step: 1,loop: !0,next: "#gscroll-ctrl-next",prev: "#gscroll-ctrl-prev",data: pageConfig.DATA_GScroll,template: pageConfig.TPL_GScroll}, function(t, e, i, a) {
    a.find("img").each(function() {
        var t = $(this).attr("data-lazyload");
        $(this).attr("style") || $(this).attr("style", t).removeAttr("data-lazyload")
    })
