        ...
        var h = function(a) {
            ...
            switch (e.source = null == e.source ? b.attr("href") : e.source, e.type) {
                ...
                case "html":
                    ...
                    break;
                case "image":
                    e._index = null == e._index ? f.index(b) : e._index, a(".thickcon").append("<img src='../../js/" + e.source + "' width='" + e.width + "' height='" + e.height + "'>"), e.source = null, a(".thickcon").removeClass(e._loading), c && c();
                    break;
                case "ajax":
                case "json":
        ...
        var c = null, b = b.join(","), d = "http://qiang.jd.com/HomePageNewLimitBuy.ashx?callback=?&ids=" + b, e = "http://item.jd.com/", f = function(b) {
            ...
            if (!(b == {} || "" == b || null == c.start || "" == c.start || null == c.end || "" == c.end || c.pros.length < 1)) {
                ...
                var d, f, h, i, j = (c.start - c.server) / 1e3, k = (c.end - c.server) / 1e3, l = function() {
                    var b = '<li><div class="p-img"><a href="../../js/{6}{0}.html" target="_blank"><img src="{1}" width="100" height="100" /></a>{2}</div><div class="p-name"><a href="../../js/{6}{0}.html" target="_blank">{3}</a></div><div class="p-price">\u62a2\u8d2d\u4ef7\uff1a<strong>{4}</strong>{5}</div></li>', d = "<ul>";
...
var jdRecent = {element: $("#recent ul"),jsurl: "http://www.jd.com/lishiset.aspx?callback=jdRecent.setData&id=",cookiename: "_recent",list: $.cookie("_recent"),url: location.href,init: function() {
        ...
        this.element.append("<li><div class='p-img'><a href='../../js/" + a.url + "'><img src='../../js/" + a.img + "' /></a></div><div class='p-name'><a href='../../js/" + a.url + "'>" + decodeURIComponent(a.name) + "</a></div></li>")
    }};
...
var category = {OBJ: $("#_JD_ALLSORT"),
		//URL_Serv: "http://manage.atguigu.com/rest/item/cat/all?callback=category.getDataService",
		URL_Serv:"http://manage.atguigu.com/rest/item/cat/all?callback=category.getDataService",
        ...
        return "" == c ? d : (/^http[s]?:\/\/([\w-]+\.)+[\w-]+([\w-.\/?%&=]*)?$/.test(c) || (c = c.replace(/-000$/, ""), c = c.match(/^\d*-\d*$/) ? "http://channel.jd.com/" + c + ".html" : "http://www.atguigu.com" + c + ".html"), '<a href="../../js/' + c + '">' + d + "</a>")
      ...
      //  $.getJSONP(this.URL_BrandsServ, category.getBrandService)
        ...
        var b = "", c = "", d = "", e = "";
        return b = this.FN_SetLink(a), c = '{if p.length!=0}<dl class="categorys-promotions">    <dd>        <ul>            {for item in p}            <li>                <a href="../../js/${item.u}" target="_blank">                {if item.i}                    <img src="../../js/${item.i}" width="194" height="70" title="${item.n}" style="margin-bottom: 4px;" />                {else}                    ${item.n}                {/if}                </a>            </li>            {/for}        </ul>    </dd></dl>{/if}', d = '{if b.length!=0}<dl class="categorys-brands">    {if id=="k"}        <dt>\u63a8\u8350\u54c1\u724c\u51fa\u7248\u5546/\u4e66\u5e97</dt>    {else}        {if id=="l"}        <dt>\u63a8\u8350\u4ea7\u54c1</dt>        {else}        <dt>\u63a8\u8350\u54c1\u724c</dt>        {/if}    {/if}    <dd>        <ul>            {for item in b} <li><a href="../../js/${item.u}" target="_blank">${item.n}</a></li> {/for}        </ul>    </dd></dl>{/if}', e = /c|b|d/.test(a) ? b + d + c : b + c + d
